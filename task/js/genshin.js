...
//请求，配置cookie和uid
let uid = 232614081;
let cookie = "ltoken=pqucjy8uvp8wYEQwh3sZx3sh1NTPnaHZ5EWdeuj0; ltuid=328270459; account_id=328270459;";
...
post("http://hn216.api.yesapi.cn/api/App/Common_Safety/Md5").then(md5 => {
  ...
  $task.fetch(myRequest).then(response => {
    ...
    //检查响应数据
    let retcode = JSON.parse(response.body).retcode;
    if (retcode != 0) {
      $notify("数据获取失败", "retcode：" + retcode, "请自行检查原因");
      //push(pushUrl, "数据获取失败", "retcode：" + retcode + "，请自行检查原因", pushIcon);
...
    }
    ...
    if (isResin) {
      ...
      if (resinMode == 1) {
        if (resin >= resinNum) {
          if(pushMode == 1){
           $notify("心怡姐姐体力提醒", "当前体力：" + resin, "已超过设定数值，请尽快使用。预计还有" + resinTime + "完全恢复。");
         }else if(pushMode == 2){
        ...
        let resinType = getCache("resinTypeXinyi");
        if (resin >= resinNum) {
    ...
    if (isHomeCoin) {
      ...
      if (homeCoinMode == 1) {
        if (homeCoin >= homeCoinNum) {
          if(pushMode == 1){
           $notify("心怡姐姐洞天宝钱提醒", "当前洞天宝钱：" + homeCoin, "已超过设定数值，请尽快使用。预计还有" + homeCoinTime + "完全恢复。");
         }else if(pushMode == 2){
...
let homeCoinType = getCache("homeCoinTypeXinyi");
        if (homeCoin >= homeCoinNum) {
...
let todayTaskReceivedType = getCache("todayTaskReceivedTypeXinyi")
        if(currentHour >= todayTaskReceivedTime && !task){
          if(todayTaskReceivedType == "未通知" || todayTaskReceivedType == undefined){
           ...
           push(pushUrl, "每日委托提醒", "每日委托奖励还没有领取哦。", pushIcon);
            ...
            setCache("todayTaskReceivedTypeXinyi","已通知");
          }else if(todayTaskReceivedType == "已通知"){
            console.log("每日委托未领取，本次不提醒");
          }
        }else if (task){
          setCache("todayTaskReceivedTypeXinyi","未通知");
        }
      }
...
let transformerType = getCache("transformerTypeXinyi");
        if(reached){
          if (transformerType == "未通知" || transformerType == undefined) {
           ...
           push(pushUrl, "参量质变仪提醒", "可以使用啦！", pushIcon);
            ...
            setCache("transformerTypeXinyi","已通知");
          } else if(transformerType == "已通知"){
            console.log("参量质变仪可以使用了，本次不提醒");
          }
        }else{
          setCache("transformerTypeXinyi","未通知");
        }
      }
