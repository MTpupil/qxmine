...
//从boxjs获取参数
const from = $.getdata("mtpupil_superTopics_from");
...
//定义请求url
const query = "from=" + from + "&s=" + s + "&c=" + c + "&request_url=" + request_url + "&gsid=" + gsid;

const sign_url = "https://api.weibo.cn/2/page/button?" + query;
const panel_url = "https://api.weibo.cn/2/page/panel?" + query;
...
//执行请求
let signResult = await $.http.get(sign).then(response => {
    let signobj = JSON.parse(response.body);
    let signmsg = signobj.button.name;
    if(signmsg != ""){
        $.log("签到成功");
        return $.http.get(panel).then(response => {
            let panelobj = JSON.parse(response.body);
